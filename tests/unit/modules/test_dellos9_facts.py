...
from ansible_collections.sense.dellos9.tests.unit.modules.dellos9_module import (
    TestDellOS9Module, load_fixture, set_module_args)
...
class TestDellOS9Facts(TestDellOS9Module):
    ...
    def test_dellos9_facts_gather_subset_default(self):
        ...
        test_data = {
            "Port-channel 104": {
                "bandwidth": 200000,
                "channel-member": ["hundredGigE " "1/17", "hundredGigE " "1/19"],
            ...
            "Vlan 101": {
                ...
                "SENSE-Rucio XRootD "
                "fff1 IPv6 Range",
                "ipv6": [{"address": "2605:d9c0:2:fff1::1", "masklen": 64}],
                "lineprotocol": "up",
                "macaddress": "4c:76:25:e8:44:c2",
                "mtu": 9416,
                "operstatus": "up",
        ...
        for key, vals in test_data.items():
            ...
            for subkey, subval in vals.items():
                self.assertEqual(
                    subval, ansible_facts["ansible_net_interfaces"][key][subkey]
                )
    ...
    def test_dellos9_facts_gather_subset_routing(self):
        ...
        test_data = {
            "ansible_net_ipv4": [
                {"from": "192.84.86.238", "to": "0.0.0.0/0", "vrf": "lhcone"}
            ],
            "ansible_net_ipv6": [
                {"from": "2605:d9c0:0:ff02::", "to": "::/0", "vrf": "lhcone"},
                {"intf": "NULL 0", "to": "2605:d9c0:2::/48", "vrf": "lhcone"},
            ],
        }
        self.assertIn("ansible_net_ipv4", ansible_facts)
        self.assertEqual(
            test_data["ansible_net_ipv4"], ansible_facts["ansible_net_ipv4"]
        )
